// Shared header-only helpers for parsing the rinha JSON payload into the
// 14-float feature vector and turning a query vector into a fraud_count 0..5.
//
// This is the same code that lives inside ext/fraud_index/fraud_index.cpp but
// extracted into a header so the standalone C++ server (native/server.cpp)
// can reuse it without depending on Ruby. The Ruby extension still has its
// own copy to keep compilation independent of this header; both should stay
// in sync byte-for-byte until we refactor the extension to include this file.
...
inline float mcc_risk(uint32_t packed) {
  switch (packed) {
...
inline int sakamoto_dow(int y, int m, int d) {
  int ya = m < 3 ? y - 1 : y;
...
inline bool id_in_known(const uint8_t *body, size_t id_s, size_t id_e,
...
inline void parse_payload(const char *raw, size_t len, float *out) {
  ...
  if (has_lt) {
    ...
    out[5] = detail::clamp01f(delta_min * detail::INV_MAX_MINUTES);
    out[6] = detail::clamp01f(lt_km     * detail::INV_MAX_KM);
  }

  if (mid_e > mid_s && detail::id_in_known(body, mid_s, mid_e, km_s, km_e)) {
    out[11] = 0.0f;
  }
  out[12] = detail::mcc_risk(mcc_packed);
}

// Counts how many of the top-5 nearest references are labeled fraud.
inline int fraud_count(const IvfIndex &idx, const float *query, int nprobe) {
  return static_cast<int>(ivf_score(idx, query, nprobe) * static_cast<float>(TOP_K) + 0.5f);
}

// Convenience: parse + score from raw payload bytes.
inline int fraud_count_payload(const IvfIndex &idx, const char *raw,
                               size_t len, int nprobe) {
  float q[DIM];
  parse_payload(raw, len, q);
  return fraud_count(idx, q, nprobe);
}
